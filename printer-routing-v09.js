...
(() => {
  ...
  function browserPrint(html,route,title){
    ...
    const receiptEl=route==='receipt'?(()=>{const d=document.createElement('div');d.style.cssText='position:absolute;left:-9999px;top:0;width:76mm;visibility:hidden';d.innerHTML=html;document.body.appendChild(d);const el=d.querySelector('.receipt')||d;const px=Math.ceil(el.scrollHeight);d.remove();return px})():0;
    const receiptMm=route==='receipt'?Math.max(60,Math.ceil(receiptEl*25.4/96)+8):0;
    const pageSize=route==='receipt'?('80mm '+receiptMm+'mm'):'A4';
